...
%         xf(i,1) = feval(fctname, x(i,:), userdata);
        xf(i,1) = Model.FunObj_Cal_HEC_HMS(x(i,:));
    end
else
    for i=1:npt
%         xf(i,1) = feval(fctname, x(i,:), userdata);
    ...
    if parallel
        ...
        parfor igs = 1: ngs
            ...
            for j = 1:nspl
                ...
                [snew, fnew, Ficall{igs}(j)]=cce_ua(Model,s,sf,bl,bu,0,userdata);
            ...
            for loop=1:nspl
                ...
                for k3=2:nps
                    for iter=1:1000
                        ...
                        idx = (lcs(1:k3-1)==lpos); if isempty(idx); break; end
                    end
                    lcs(k3) = lpos;
                end
                lcs=sort(lcs);
                ...
                [snew,fnew,icall]=cce_ua(Model,s,sf,bl,bu,icall,userdata);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [snew,fnew,icall]=cce_ua(Model,s,sf,bl,bu,icall,userdata)
...
ibound=0;
s1=snew-bl; idx=(s1<0); if ~isempty(idx); ibound=1; end
s1=bu-snew; idx=(s1<0); if ~isempty(idx); ibound=2; end
...
fnew = Model.FunObj_Cal_HEC_HMS(snew);
